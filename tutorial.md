...
> 本文着重点在于语法与机制，**代码示例不涉及任何第三方库，只涉及标准库**，因此**本教程并不能教会你使用Python做爬虫、人工智能、大数据、Web开发等**。\
...
[标注]仅用于[函数]的[参数列表]声明。[标注]的主要作用是表示[函数]的[参数]和[返回值]的[类型]。你可以通过[函数] [对象]的`__annotations__`[属性]获取[函数]的所有[标注]（是一个[字典]），这个[字典]的[键]是[函数]的[参数]名，以及一个特殊的`return`，值就是这个[参数]（`return`对应[返回值]）的[标注]。
...
注意：此处的`p2: 1 + 2 = 9`同时包含了对p2进行的[标注]和p2的默认值。

可以注意到，[参数]的[标注]用冒号`:`分隔，[返回值]的标注用`->`分隔，并且写在被[标注]的[参数]/[返回值]的后面。
...
如果你使用`Visual Studio`或者`Visual Studio Code`作为`Python`的编辑器，为自定义[函数]添加合适的[标注]将获得相应的`代码提示`（即编辑器能够借助[标注]作出`类型推断`）。借助[标注]，我们可以在`Python`这样的脚本语言中使用安全、准确的`类型推断`与`代码提示`，还能提高代码的**可读性**，何乐而不为？

> 请注意[标注]并**不能影响**[函数调用]。即使[函数调用]时实际传入的[参数]不符合标注声明的[类型]（例如上例的p1应当是int[类型]）或者值（例如上例的p2应当被传入值3），`Python`解释器也**不会为此做出任何类型/值检查**。
